const { REGEX, CODES, MESSAGES } = require('../../constants')
...
module.exports = {
    ...
    validateCreateUrl: async (req, res, next) => {
        let schema = joi.object().keys({
        ...
        if (req == "swagger") {
            ...
            let summary = "API to Create shortUrl"
            let description = "API to Create shortUrl"
            let swagger = {
                example: example,
                schema: schema,
                querySchema: typeof querySchema !== 'undefined' ? querySchema : false,
    ...
    isAdminValid: async (req, res, next) => {
        try {
            if (req.user && req.user.guestMode) {
                ...
                const userData = await MODELS.user.findOne({ _id: decodeData._id, type: 0 }).lean().exec();
